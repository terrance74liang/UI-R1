...
def extract_bbox(response):
    ...
    coord_match = re.search(bbox_pattern, response)
    if coord_match:
...
def run(rank, world_size, args):
    ...
    for json_file in json_files:
        ...
        for j, item in tqdm(enumerate(data), total=len(data)):
            ...
            question_template = 'Outline the position corresponding to the instruction: {}. The output should be only [x1,y1,x2,y2].'.format(task_prompt)
            # w/o thinking
            # question_template = (
